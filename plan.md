...
status: active
updated: 2026-06-23
goal_version: 65a7bb081d7e
acceptance:
  - id: D2-case-input
    hint: "The case-creation form submits identifying terms to a Next.js API route, which persists them as a row in a Prisma `Case` model with an `identifyingTerms` text column on Vercel Postgres; a single migration under prisma/migrations/ creates the Case table; the saved case is read back and shown in the case list; empty or whitespace-only input is rejected with no row written. next build and tsc --noEmit both pass."
    high_impact: true
  - id: PKG-HEALTH
...
- D2-case-input: The case-creation form submits identifying terms to a Next.js API route, which persists them as a row in a Prisma `Case` model with an `identifyingTerms` text column on Vercel Postgres; a single migration under prisma/migrations/ creates the Case table; the saved case is read back and shown in the case list; empty or whitespace-only input is rejected with no row written. next build and tsc --noEmit both pass.
