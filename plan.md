...
status: active
updated: 2026-06-23
goal_version: 7240b95f9883
acceptance:
  - id: D2-case-input
    hint: "Submitting identifying terms in the case-creation form calls a Next.js API route that persists them as a row in a Prisma `Case` model whose `identifyingTerms` text column lives on Vercel Postgres; a single migration under prisma/migrations/ creates the Case table; afterward the saved case is read back and shown in the case list; empty or whitespace-only input is rejected and no row is written. Both next build and tsc --noEmit pass."
    high_impact: true
  - id: PKG-HEALTH
...
A user can enter the identifying terms (their name/identifier) to search on, saved as a case.
...
- D2-case-input: Submitting identifying terms in the case-creation form calls a Next.js API route that persists them as a row in a Prisma `Case` model whose `identifyingTerms` text column lives on Vercel Postgres; a single migration under prisma/migrations/ creates the Case table; afterward the saved case is read back and shown in the case list; empty or whitespace-only input is rejected and no row is written. Both next build and tsc --noEmit pass.
