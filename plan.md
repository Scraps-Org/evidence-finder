...
status: active
updated: 2026-06-22
goal_version: 51b151ebe5a7
acceptance:
  - id: D2-case-input
    hint: "A form accepts identifying terms; on submit, a Next.js API route persists a row to a Prisma `Case` model on Vercel Postgres (a `prisma migrate` migration creates the table); the saved case is then read back and shown in the case list; empty or whitespace-only input is rejected with no row written. build + tsc --noEmit pass."
    high_impact: true
  - id: PKG-HEALTH
...
- D2-case-input: A form accepts identifying terms; on submit, a Next.js API route persists a row to a Prisma `Case` model on Vercel Postgres (a `prisma migrate` migration creates the table); the saved case is then read back and shown in the case list; empty or whitespace-only input is rejected with no row written. build + tsc --noEmit pass.
