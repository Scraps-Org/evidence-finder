...
status: active
updated: 2026-06-22
goal_version: b7603b903386
acceptance:
  - id: D2-case-input
    hint: "The case-creation form accepts identifying terms and, on submit, a Next.js API route inserts a row into a Prisma `Case` model (an `identifyingTerms` text column) on Vercel Postgres (a single migration under prisma/migrations/ creates the Case table); the newly saved case is read back and appears in the case list; empty or whitespace-only input is rejected and writes no row. build + tsc --noEmit both pass."
    high_impact: true
  - id: PKG-HEALTH
...
- D2-case-input: The case-creation form accepts identifying terms and, on submit, a Next.js API route inserts a row into a Prisma `Case` model (an `identifyingTerms` text column) on Vercel Postgres (a single migration under prisma/migrations/ creates the Case table); the newly saved case is read back and appears in the case list; empty or whitespace-only input is rejected and writes no row. build + tsc --noEmit both pass.
