...
status: active
updated: 2026-06-23
goal_version: e82fe0e49946
acceptance:
  - id: D2-case-input
    hint: "When identifying terms are submitted, a Next.js API route writes them as a row in a Prisma `Case` model that has an `identifyingTerms` text column on Vercel Postgres, with a single migration under prisma/migrations/ creating the Case table; the saved case is then read back and shown in the case list; empty or whitespace-only input is rejected without writing a row. next build and tsc --noEmit must both pass."
    high_impact: true
  - id: PKG-HEALTH
...
- D2-case-input: When identifying terms are submitted, a Next.js API route writes them as a row in a Prisma `Case` model that has an `identifyingTerms` text column on Vercel Postgres, with a single migration under prisma/migrations/ creating the Case table; the saved case is then read back and shown in the case list; empty or whitespace-only input is rejected without writing a row. next build and tsc --noEmit must both pass.
