...
status: active
updated: 2026-06-21
goal_version: 3d4b6c4808b7
acceptance:
  - id: D2-case-input
    hint: "A form accepts identifying terms and on submit a Next.js API route persists a row to a Prisma `Case` model on Vercel Postgres (a `prisma migrate` adds the table); the saved case is read back and rendered in a list; empty or whitespace-only input is rejected and no row is written. build + tsc pass."
    high_impact: true
  - id: PKG-HEALTH
...
- D2-case-input: A form accepts identifying terms and on submit a Next.js API route persists a row to a Prisma `Case` model on Vercel Postgres (a `prisma migrate` adds the table); the saved case is read back and rendered in a list; empty or whitespace-only input is rejected and no row is written. build + tsc pass.
