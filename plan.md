...
status: active
updated: 2026-06-23
goal_version: bfb3a5c0e0c9
acceptance:
  - id: D2-case-input
    hint: "On submit, the case-creation form's identifying terms are inserted by a Next.js API route as a row in a Prisma `Case` model (an `identifyingTerms` text column) on Vercel Postgres; a single migration under prisma/migrations/ creates the Case table; the saved case is then read back and appears in the case list; empty or whitespace-only input is rejected and writes no row. next build + tsc --noEmit both pass."
    high_impact: true
  - id: PKG-HEALTH
...
- D2-case-input: On submit, the case-creation form's identifying terms are inserted by a Next.js API route as a row in a Prisma `Case` model (an `identifyingTerms` text column) on Vercel Postgres; a single migration under prisma/migrations/ creates the Case table; the saved case is then read back and appears in the case list; empty or whitespace-only input is rejected and writes no row. next build + tsc --noEmit both pass.
