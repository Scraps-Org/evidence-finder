...
describe('D9-recall-and-referral: recall-limit notice and referral links', () => {
  it('renders a static notice asserting public-index-only coverage and manual-reporting requirement for closed platforms', async () => {
    ...
    const coversPublicIndex =
      /공개.{0,30}색인|publicly.{0,30}index|public.{0,30}index/i.test(pageText)
    expect(
      coversPublicIndex,
    ...
    const coversClosedPlatforms =
      /텔레그램|telegram|포럼|forum|폐쇄|closed.{0,20}platform|manual.{0,20}report|수동.{0,10}신고/i.test(pageText)
    expect(
  ...
  it('renders individual navigable links to StopNCII.org, NCMEC Take It Down, and Google explicit-image removal', async () => {
    ...
    const hasGoogle = hrefs.some(h => /support\.google\.com|google\.com.*remov|google\.com.*explicit/i.test(h))
  ...
  it('referral links are plain anchors/navigation links only — none invoke, embed, or trigger scanning', async () => {
    ...
    for (const pattern of referralPatterns) {
      ...
      expect(
        ...
        `Referral link matching ${pattern} must exist`
      ).toBeGreaterThan(0)
      ...
      for (const link of matchingLinks) {
        ...
        expect(
          ...
          'Referral must be rendered as an <a> element'
        ).toBe('a')
        expect(
          anchor.href,
          'Referral <a> must have a non-empty href'
        ).toBeTruthy()
